...
- NextJS: yeah sure chắc chắn là như vậy rồi. Và để update mới nhất, mình dùng NextJS 14.2.
...
- Còn về UI, thì mình sử dụng [TailwindCSS](https://tailwindcss.com/) (chủ yếu là chép từ [^1] thôi) và không thể thiếu là thư viện [shadcn/ui](https://ui.shadcn.com/) (các button, link, animation gì đó đều được mình lấy từ thư viện này hết).
...
```ts showLineNumbers title="/lib/api.ts"
...
- Phần hay nữa đó chính là mình hoàn toàn có thể dùng React Component (jsx hoặc tsx) trong MDX bằng cách thêm vào ở option `components` trong `MDXRemote` component. Ví dụ như mình dùng `<MdxMathBlock />` để tạo các block math như Definition, Theorem, ... như dưới đây.
...
- Cool ha, việc sử dụng được React Component nên mình cảm giác nhét gì vào cũng được 🤣 (nhưng đủ khả năng làm không, không). Hay mình có thể dùng `<MdxYoutube />` để embed video youtube vào.

```tsx
<MdxYoutube
  src="abPmZCZZrFA"
  title="Đừng làm trái tim anh đau"
  caption="Đừng làm trái tim anh đau"
/>
```
...
- [x] Add Google Analytics and Search Console.
- [ ] Add SEO.
- [x] Better Table of Contents (maybe nằm ở bên phải or trái, mình quyết định bỏ luôn, nhìn hơi ngứa mắt 😑).
- [ ] Post Search.
...
- [ ] Post Comment.
- [ ] Dùng database.
- [x] Improve MDX Loading Time (mình không biết có thể gọi là improve không, mình dùng `generateStaticParams` [^2] trong NextJS để build các post này thành HTML ở build time luôn, gọi là Static Site Generation [^3]).
- [ ] Sitemap.

[^1]: https://leerob.io/
[^2]: https://nextjs.org/docs/app/api-reference/functions/generate-static-params
[^3]: https://developer.mozilla.org/en-US/blog/static-site-generation-with-nextjs/