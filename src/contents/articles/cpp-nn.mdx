...
```cpp
// việc dùng using cũng giống như #define để gõ nhanh hơn :>
// Ở đây mình không truyền thẳng class Matrix vào 
// mà truyền std::vector<std::vector<double>>
// là vì cách mình cài đặt :>
// xem source code sẽ rõ hơn.
using MatrixType = std::vector<std::vector<double>>;

MatrixType SoftMax(const MatrixType& x)
{
    // mình giả sử ma trận truyền vào có cột = 1 (xem như 1 vector)
