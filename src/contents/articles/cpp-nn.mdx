---
title: Cùng mình xây dựng Neural Network đơn giản (Phần 1)
create_date: 07/15/2024
update_date: 07/27/2024
tags:
    - deep learning
draft: false
---

<MdxUpdate title="Các cập nhật">
<ul>
    <li>25/07/2024: Nhắc sơ qua phần bias. </li>
    <li>26/07/2024: Thêm implementation các hàm activation và Softmax. </li>
    <li>27/07/2024: Sửa lỗi sai ở activation Sigmoid và ở phần Matrix::Randomized.</li>
</ul>
</MdxUpdate>
...
Mình sẽ có 1 file `CMakeLists.txt` xem như một build generator, dùng để build toàn bộ source code của mình. Source code sẽ chia làm hai phần là `src` chứa các file `.cpp` còn `include` sẽ chứa các file `.h`. Nếu các bạn dùng Visual Studio có thể skip đi phần Cmake này. */}
...
Tiếp theo là mình phải tìm cách để khởi tạo weight cho Neural Network một cách tối ưu nhất, theo [^5] thì khởi tạo trọng số như sau: Với mỗi trọng số $w$, ta lấy $w$ từ một phân phối chuẩn có trung bình là $0$ và phương sai là $2/n$ với $n$ là số chiều của vector input (hay đúng hơn là chiều của layer phía trước).
...
Trước khi kết thúc thì còn 1 hàm activation rất quan trọng nữa đó chính là hàm **Softmax** [^7]. Softmax cũng là một activation nhưng thay vì truyền vào một số thực như các hàm khác thì ta sẽ truyền 1 vector và cho Softmax và output của Softmax cũng là một vector (do đó không còn chuyện element-wise như trên nữa).

Xét một vector bất kì gồm $N$ phần tử $\mathbf{x} = \{x_1, x_2, ..., x_N\}$. Khi đó:
$$
\text{softmax}(\mathbf{x})_i = \dfrac{e^{x_i}}{\sum_{j=1}^N e^{x_j}}
$$
hay:
$$
\text{softmax}(\mathbf{x}) =\left\{ \text{softmax}(\mathbf{x})_1, ..., \text{softmax}(\mathbf{x})_N \right\}
$$

Có thể thấy các phần tử của vector $\text{softmax}$ luôn có giá trị nằm trong khoảng $(0, 1)$ ngoài ra tổng các phần tử của vector bằng $1$ do đó vector $\text{softmax}$ này có thể được người ta xem như vector phân phối vậy, trong đó mỗi phần tử được xem như là xác suất. Ngoài ra hàm activation Softmax thường được áp dụng cho layer cuối, khi đó output cho ra sẽ là xác suất. Ví dụ như output layer cuối cùng gồm 10 phần tử, ta xem mỗi phần tử ấy là 1 lớp, sau khi qua Softmax thì mỗi lớp sẽ có giá trị xác suất tương ứng của mình.
...
> Vậy là xong class Ma trận và các hàm phụ trợ rồi ấy, cảm ơn các bạn đã đọc đến đây hehe 🍉. Ở phần sau [Cùng mình xây dựng Neural Network đơn giản (Phần 2)](/blog/cpp-nn-2) ta sẽ đi qua kỹ thuật Backpropagation (hay còn gọi là Backward) và cách train Deep Neural Network.
